...
export default function NewClimbPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!zoneId) { setError('No zone specified.'); return }
    setSaving(true)
    setError(null)
    ...
    const { error: insertError } = await supabase.from('climbs').insert(insertData)
    if (insertError) {
  ...
  return (
    <div className={`${poppins.className} min-h-screen bg-zinc-950 text-zinc-100 flex flex-col`}>
      ...
      <div className="sticky top-0 z-10 bg-zinc-950/95 backdrop-blur border-b border-zinc-800 flex items-center gap-3 px-4 py-3">
        <button
          onClick={() => router.push(zoneId ? `/zone/${zoneId}` : '/dashboard')}
          className="shrink-0 text-zinc-400 hover:text-zinc-100 active:scale-90 transition-all p-0.5 -ml-0.5"
          aria-label="Go back"
        >
          <BackIcon />
        </button>
        <h1 className="flex-1 text-base font-semibold text-zinc-100">Add Climb</h1>
      </div>
